...
// GET /api/users/:id/trips — Отримати поїздки користувача (водія)
exports.getTripsByDriver = async (req, res) => {
	try {
		const driverId = parseInt(req.params.id);
		const allTrips = await tripService.fetchTrips({});

		const userTrips = allTrips.filter((trip) => trip.driverId === driverId);
